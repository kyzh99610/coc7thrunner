...
def test_web_app_experimental_ai_demo_page_loads_without_breaking_keeper_shell_when_llm_disabled(
    ...
    assert 'id="experimental-demo-observer-last-run-recall"' not in html
...
def test_experimental_autonomous_session_loop_checkpoint_recap_copy_stays_single_entry_and_small() -> None:
    checkpoint = web_app_route.ExperimentalAutonomousSessionLoopCheckpoint(
        selected_investigator_id="investigator-1",
        current_turn_index=2,
        last_batch_max_turns=2,
    ...
    recap_copy = web_app_route._build_experimental_autonomous_session_loop_recap_copy(
        checkpoint
    )
    recap_html = (
    ...
    assert recap_copy == web_app_route.ExperimentalAutonomousSessionLoopRecapCopy(
        loop_status_text="当前 loop 状态：第 1 段 bounded batch 已到达边界，可继续第 2 段。",
        continuation_count_text="已完成批次：第 1 段。",
        next_turn_text="下一段起点：第 3 轮。",
        batch_status_text="上一段状态：达到轮数上限",
        batch_stop_reason_text="上一段停止原因：达到当前受控 one-shot demo run 的最大轮数上限。",
    )
    assert 'id="experimental-demo-observer-session-loop-recap"' in recap_html
    assert "当前 loop 状态：第 1 段 bounded batch 已到达边界，可继续第 2 段。" in recap_html
    assert "上一段状态：达到轮数上限" in recap_html
...
def test_web_app_experimental_ai_demo_page_can_recall_last_autopilot_run_near_current_token_surface(
    ...
    assert html.index('id="experimental-demo-observer-last-run-recall"') > html.index(
        'id="experimental-demo-observer"'
    )
...
def test_web_app_experimental_ai_demo_observer_header_can_show_last_run_recall_and_checkpoint_recap_together(
    client: TestClient,
) -> None:
    session_id = _start_keeper_dashboard_session(client)
    _advance_keeper_dashboard_session(client, session_id)
    ...
    assert "当前 loop 状态：第 1 段 bounded batch 已到达边界，可继续第 2 段。" in html
    assert "下一段起点：第 3 轮。" in html
    assert html.index('id="experimental-demo-observer-session-loop-recap"') > html.index(
        'id="experimental-demo-observer-last-run-recall"'
    )
    after_snapshot = _get_snapshot(client, session_id)
    assert before_snapshot == after_snapshot
...
def test_experimental_one_shot_preset_internal_diagnostic_exposes_keeper_only_text_without_public_render(
    ...
    agent_turn_brief = (
        web_app_route._build_experimental_one_shot_internal_autopilot_agent_turn_brief(
            run_result=run_result
        )
    )
    agent_turn_note = (
        web_app_route._build_experimental_one_shot_internal_autopilot_agent_turn_note(
            run_result=run_result
        )
    )
    agent_turn_memo = (
        web_app_route._build_experimental_one_shot_internal_autopilot_agent_turn_memo(
            run_result=run_result
        )
    )
    agent_memo_input = (
    ...
    assert agent_input_envelope == {
        "envelope_kind": "envelope_pin_focus",
        "preset_id": "scenario.midnight_archive",
        "preset_label": "雨夜档案馆",
        "envelope_text": (
    ...
    assert agent_turn_note == {
        "note_kind": "note_pin_focus",
        "preset_id": "scenario.midnight_archive",
        "preset_label": "雨夜档案馆",
        "note_text": (
    ...
    assert agent_turn_memo == {
        "memo_kind": "memo_pin_focus",
        "preset_id": "scenario.midnight_archive",
        "preset_label": "雨夜档案馆",
        "memo_text": (
    ...
    assert agent_memo_input == {
        "input_kind": "input_pin_focus",
        "preset_id": "scenario.midnight_archive",
        "preset_label": "雨夜档案馆",
        "input_text": (
    ...
    agent_turn_brief_json = json.dumps(
        agent_turn_brief,
        ensure_ascii=False,
        separators=(",", ":"),
    )
    agent_turn_note_json = json.dumps(
        agent_turn_note,
        ensure_ascii=False,
        separators=(",", ":"),
    )
    agent_turn_memo_json = json.dumps(
        agent_turn_memo,
        ensure_ascii=False,
        separators=(",", ":"),
    )
    agent_memo_input_json = json.dumps(
        agent_memo_input,
        ensure_ascii=False,
        separators=(",", ":"),
    )
    ...
    assert agent_turn_brief_json not in html
...
def test_experimental_one_shot_internal_autopilot_agent_input_envelope_helper_returns_bounded_envelope_for_supported_presets(
    client: TestClient,
    start_session,
    advance_session,
    focus_by_turn: dict[int, str],
    expected: web_app_route.ExperimentalOneShotInternalAutopilotAgentInputEnvelope,
) -> None:
    session_id = start_session(client)
    advance_session(client, session_id)
    fake_service = _SequencedOneShotLocalLLMService(focus_by_turn=focus_by_turn)
    ...
    assert before_snapshot == _get_snapshot(client, session_id)
    assert agent_input_envelope == expected
    assert set(agent_input_envelope) == {
        "envelope_kind",
        "preset_id",
        "preset_label",
        "envelope_text",
...
def test_experimental_one_shot_internal_autopilot_agent_turn_brief_helper_returns_bounded_brief_for_supported_presets(
    client: TestClient,
    start_session,
    advance_session,
    focus_by_turn: dict[int, str],
    expected: web_app_route.ExperimentalOneShotInternalAutopilotAgentTurnBrief,
) -> None:
    session_id = start_session(client)
    advance_session(client, session_id)
    fake_service = _SequencedOneShotLocalLLMService(focus_by_turn=focus_by_turn)
    ...
    assert before_snapshot == _get_snapshot(client, session_id)
    assert agent_turn_brief == expected
    assert set(agent_turn_brief) == {
        "brief_kind",
        "preset_id",
        "preset_label",
        "brief_text",
...
def test_experimental_one_shot_internal_autopilot_agent_turn_note_helper_returns_bounded_note_for_supported_presets(
    client: TestClient,
    start_session,
    advance_session,
    focus_by_turn: dict[int, str],
    expected: web_app_route.ExperimentalOneShotInternalAutopilotAgentTurnNote,
) -> None:
    session_id = start_session(client)
    advance_session(client, session_id)
    fake_service = _SequencedOneShotLocalLLMService(focus_by_turn=focus_by_turn)
    ...
    assert before_snapshot == _get_snapshot(client, session_id)
    assert agent_turn_note == expected
    assert set(agent_turn_note) == {
        "note_kind",
        "preset_id",
        "preset_label",
        "note_text",
...
def test_experimental_one_shot_internal_autopilot_agent_turn_memo_helper_returns_bounded_memo_for_supported_presets(
    client: TestClient,
    start_session,
    advance_session,
    focus_by_turn: dict[int, str],
    expected: web_app_route.ExperimentalOneShotInternalAutopilotAgentTurnMemo,
) -> None:
    session_id = start_session(client)
    advance_session(client, session_id)
    fake_service = _SequencedOneShotLocalLLMService(focus_by_turn=focus_by_turn)
    ...
    assert before_snapshot == _get_snapshot(client, session_id)
    assert agent_turn_memo == expected
    assert set(agent_turn_memo) == {
        "memo_kind",
        "preset_id",
        "preset_label",
        "memo_text",
...
def test_experimental_one_shot_internal_autopilot_agent_memo_input_helper_returns_bounded_input_for_supported_presets(
    client: TestClient,
    start_session,
    advance_session,
    focus_by_turn: dict[int, str],
    expected: web_app_route.ExperimentalOneShotInternalAutopilotAgentMemoInput,
) -> None:
    session_id = start_session(client)
    advance_session(client, session_id)
    fake_service = _SequencedOneShotLocalLLMService(focus_by_turn=focus_by_turn)
    ...
    assert before_snapshot == _get_snapshot(client, session_id)
    assert agent_memo_input == expected
    assert set(agent_memo_input) == {
        "input_kind",
        "preset_id",
        "preset_label",
        "input_text",
...
@pytest.mark.parametrize(
    ("payload_kind", "expected_envelope_kind"),
    [
        ("payload_pin_focus", "envelope_pin_focus"),
        ("payload_advance_focus", "envelope_advance_focus"),
        ("payload_stabilize_focus", "envelope_stabilize_focus"),
    ],
)
def test_experimental_one_shot_internal_autopilot_agent_input_envelope_helper_delegates_to_executable_step_payload_helper(
    monkeypatch: pytest.MonkeyPatch,
    payload_kind: str,
    expected_envelope_kind: str,
) -> None:
    payload_calls: list[web_app_route.ExperimentalOneShotRunResult] = []
    run_result = _make_empty_experimental_one_shot_run_result()

    def _fake_payload_helper(
        *,
        run_result: web_app_route.ExperimentalOneShotRunResult,
    ) -> web_app_route.ExperimentalOneShotInternalAutopilotExecutableStepPayload:
        payload_calls.append(run_result)
        return {
            "payload_kind": payload_kind,
            "preset_id": "scenario.midnight_archive",
            "preset_label": "雨夜档案馆",
            "payload_text": "input-envelope sentinel",
        }

    monkeypatch.setattr(
        web_app_route,
        "_build_experimental_one_shot_internal_autopilot_executable_step_payload",
        _fake_payload_helper,
    )
    ...
    assert (
        ...
        == {
            "envelope_kind": expected_envelope_kind,
            "preset_id": "scenario.midnight_archive",
            "preset_label": "雨夜档案馆",
            "envelope_text": "封装为 internal agent 单步输入：input-envelope sentinel",
        }
    )
...
@pytest.mark.parametrize(
    ("turn_input_kind", "expected_brief_kind"),
    [
        ("turn_input_pin_focus", "brief_pin_focus"),
        ("turn_input_advance_focus", "brief_advance_focus"),
        ("turn_input_stabilize_focus", "brief_stabilize_focus"),
    ],
)
def test_experimental_one_shot_internal_autopilot_agent_turn_brief_helper_delegates_to_agent_turn_input_helper(
    monkeypatch: pytest.MonkeyPatch,
    turn_input_kind: str,
    expected_brief_kind: str,
) -> None:
    turn_input_calls: list[web_app_route.ExperimentalOneShotRunResult] = []
    run_result = _make_empty_experimental_one_shot_run_result()

    def _fake_agent_turn_input_helper(
        *,
        run_result: web_app_route.ExperimentalOneShotRunResult,
    ) -> web_app_route.ExperimentalOneShotInternalAutopilotAgentTurnInput:
        turn_input_calls.append(run_result)
        return {
            "turn_input_kind": turn_input_kind,
            "preset_id": "scenario.midnight_archive",
            "preset_label": "雨夜档案馆",
            "turn_input_text": "turn-brief sentinel",
        }

    monkeypatch.setattr(
        web_app_route,
        "_build_experimental_one_shot_internal_autopilot_agent_turn_input",
        _fake_agent_turn_input_helper,
    )
    ...
    assert (
        ...
        == {
            ...
            "preset_id": "scenario.midnight_archive",
            "preset_label": "雨夜档案馆",
            "brief_text": "整理为 internal agent 单轮摘要：turn-brief sentinel",
        }
    )
    assert turn_input_calls == [run_result]


@pytest.mark.parametrize(
    ("brief_kind", "expected_note_kind"),
    [
        ("brief_pin_focus", "note_pin_focus"),
        ("brief_advance_focus", "note_advance_focus"),
        ("brief_stabilize_focus", "note_stabilize_focus"),
    ],
)
def test_experimental_one_shot_internal_autopilot_agent_turn_note_helper_delegates_to_agent_turn_brief_helper(
    monkeypatch: pytest.MonkeyPatch,
    brief_kind: str,
    expected_note_kind: str,
) -> None:
    turn_brief_calls: list[web_app_route.ExperimentalOneShotRunResult] = []
    run_result = _make_empty_experimental_one_shot_run_result()

    def _fake_agent_turn_brief_helper(
        *,
        run_result: web_app_route.ExperimentalOneShotRunResult,
    ) -> web_app_route.ExperimentalOneShotInternalAutopilotAgentTurnBrief:
        turn_brief_calls.append(run_result)
        return {
            "brief_kind": brief_kind,
            "preset_id": "scenario.midnight_archive",
            "preset_label": "雨夜档案馆",
            "brief_text": "turn-note sentinel",
        }

    monkeypatch.setattr(
        web_app_route,
        "_build_experimental_one_shot_internal_autopilot_agent_turn_brief",
        _fake_agent_turn_brief_helper,
    )
    ...
    assert (
        ...
        == {
            ...
            "preset_id": "scenario.midnight_archive",
            "preset_label": "雨夜档案馆",
            "note_text": "整理为 internal agent 单轮便签：turn-note sentinel",
        }
    )
    assert turn_brief_calls == [run_result]


@pytest.mark.parametrize(
    ("note_kind", "expected_memo_kind"),
    [
        ("note_pin_focus", "memo_pin_focus"),
        ("note_advance_focus", "memo_advance_focus"),
        ("note_stabilize_focus", "memo_stabilize_focus"),
    ],
)
def test_experimental_one_shot_internal_autopilot_agent_turn_memo_helper_delegates_to_agent_turn_note_helper(
    monkeypatch: pytest.MonkeyPatch,
    note_kind: str,
    expected_memo_kind: str,
) -> None:
    turn_note_calls: list[web_app_route.ExperimentalOneShotRunResult] = []
    run_result = _make_empty_experimental_one_shot_run_result()

    def _fake_agent_turn_note_helper(
        *,
        run_result: web_app_route.ExperimentalOneShotRunResult,
    ) -> web_app_route.ExperimentalOneShotInternalAutopilotAgentTurnNote:
        turn_note_calls.append(run_result)
        return {
            "note_kind": note_kind,
            "preset_id": "scenario.midnight_archive",
            "preset_label": "雨夜档案馆",
            "note_text": "turn-memo sentinel",
        }

    monkeypatch.setattr(
        web_app_route,
        "_build_experimental_one_shot_internal_autopilot_agent_turn_note",
        _fake_agent_turn_note_helper,
    )
    ...
    assert (
        ...
        == {
            "memo_kind": expected_memo_kind,
            "preset_id": "scenario.midnight_archive",
            "preset_label": "雨夜档案馆",
            "memo_text": "整理为 internal agent 单轮 memo：turn-memo sentinel",
        }
    )
    assert turn_note_calls == [run_result]


@pytest.mark.parametrize(
    ("memo_kind", "expected_input_kind"),
    [
        ("memo_pin_focus", "input_pin_focus"),
        ("memo_advance_focus", "input_advance_focus"),
        ("memo_stabilize_focus", "input_stabilize_focus"),
    ],
)
def test_experimental_one_shot_internal_autopilot_agent_memo_input_helper_delegates_to_agent_turn_memo_helper(
    monkeypatch: pytest.MonkeyPatch,
    memo_kind: str,
    expected_input_kind: str,
) -> None:
    turn_memo_calls: list[web_app_route.ExperimentalOneShotRunResult] = []
    run_result = _make_empty_experimental_one_shot_run_result()

    def _fake_agent_turn_memo_helper(
        *,
        run_result: web_app_route.ExperimentalOneShotRunResult,
    ) -> web_app_route.ExperimentalOneShotInternalAutopilotAgentTurnMemo:
        turn_memo_calls.append(run_result)
        return {
            "memo_kind": memo_kind,
            "preset_id": "scenario.midnight_archive",
            "preset_label": "雨夜档案馆",
            "memo_text": "memo-input sentinel",
        }

    monkeypatch.setattr(
        web_app_route,
        "_build_experimental_one_shot_internal_autopilot_agent_turn_memo",
        _fake_agent_turn_memo_helper,
    )
    ...
    assert (
        ...
        == {
            ...
            "preset_id": "scenario.midnight_archive",
            "preset_label": "雨夜档案馆",
            "input_text": "封装为 internal agent memo 输入：memo-input sentinel",
        }
    )
...
def test_experimental_one_shot_internal_autopilot_agent_input_envelope_helper_returns_none_without_executable_step_payload(
) -> None:
    run_result = _make_empty_experimental_one_shot_run_result(
        scenario_preset_internal_diagnostic=None,
        scenario_preset_internal_diagnostic_json="",
...
def test_experimental_one_shot_internal_autopilot_agent_turn_brief_helper_returns_none_without_agent_turn_input(
) -> None:
    run_result = _make_empty_experimental_one_shot_run_result(
        scenario_preset_internal_diagnostic=None,
        scenario_preset_internal_diagnostic_json="",
...
def test_experimental_one_shot_internal_autopilot_agent_turn_note_helper_returns_none_without_agent_turn_brief(
) -> None:
    run_result = _make_empty_experimental_one_shot_run_result(
        scenario_preset_internal_diagnostic=None,
        scenario_preset_internal_diagnostic_json="",
...
def test_experimental_one_shot_internal_autopilot_agent_turn_memo_helper_returns_none_without_agent_turn_note(
) -> None:
    run_result = _make_empty_experimental_one_shot_run_result(
        scenario_preset_internal_diagnostic=None,
        scenario_preset_internal_diagnostic_json="",
...
def test_experimental_one_shot_internal_autopilot_agent_memo_input_helper_returns_none_without_agent_turn_memo(
) -> None:
    run_result = _make_empty_experimental_one_shot_run_result(
        scenario_preset_internal_diagnostic=None,
        scenario_preset_internal_diagnostic_json="",
